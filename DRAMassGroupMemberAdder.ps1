...
Input:
    ...
    $destinationGroup
        Which group all the computers from todolist will go.
...
$Server = $null,
$destinationGroup = (Read-Host -Prompt "What's the destination Group?")
)
...
Write-Host -Object ("Detected " + $todolist.count + " computers to add in $destinationGroup. Continue?")
Pause


Write-Host "Starting add. If you get a lot of 500 internal errors, choose a different server."
...
foreach ($comp in $todolist)
{
    ...
    if (Add-DRAGroupMembers -Domain $env:USERDNSDOMAIN -Identifier $destinationGroup -Computers $comp -Force)
    {
        $result.result = "Success"
        $CompletionList += $result
        Write-Host "[*] $comp was moved to $destinationGroup"
    }
    else
    {
        $result.result = "Fail"
